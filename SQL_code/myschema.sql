...
(
    ...
    expiration_date DATE CHECK (expiration_date >= start_date),
...
BEGIN
    IF NEW.status_ = 'applied' THEN
        SET NEW.start_date = CURRENT_DATE();
        SET NEW.expiration_date = DATE_ADD(CURRENT_DATE(), INTERVAL 3 MINUTE);
    END IF;
END;
...
CREATE TRIGGER trigger_update_active_borrows_on_borrowing
    BEFORE INSERT
    ON applications
    FOR EACH ROW
BEGIN
...
CREATE TRIGGER trigger_update_active_borrows_on_delete
    AFTER DELETE
    ON applications
    FOR EACH ROW
BEGIN
